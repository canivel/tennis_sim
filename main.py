...
class FastOddsPredictor:
    def __init__(self, model_path='tennis_model.joblib'):
        model_data = load(model_path)
        self.pipeline = model_data['pipeline']
        self.feature_names = model_data['feature_names']

    def prepare_features(self, match_state: MatchState, player_stats: List[PlayerStats]) -> pd.DataFrame:
        features = [
            match_state.set_score[0], match_state.set_score[1],
            match_state.game_score[0], match_state.game_score[1],
            player_stats[0].serve_accuracy, player_stats[1].serve_accuracy,
            player_stats[0].groundstroke_accuracy, player_stats[1].groundstroke_accuracy,
            match_state.player_fatigue[0], match_state.player_fatigue[1]
        ]
        return pd.DataFrame([features], columns=self.feature_names)

    def predict_win_probability(self, features: pd.DataFrame) -> float:
        return self.pipeline.predict_proba(features)[0][1]  # Probability of player 1 winning

class OddsCalculator:
    @staticmethod
    def calculate_odds(probability: float) -> Tuple[float, float]:
        if probability <= 0 or probability >= 1:
            return 1.01, 1.01  # Avoid division by zero
        return 1 / probability, 1 / (1 - probability)

    @staticmethod
    def adjust_odds(current_odds: List[float], new_probability: float, weight: float = 0.1) -> List[float]:
        current_prob = 1 / current_odds[0]
        adjusted_prob = (1 - weight) * current_prob + weight * new_probability
        return list(OddsCalculator.calculate_odds(adjusted_prob))
...
class TennisMatch:
    def __init__(self, player1: PlayerStats, player2: PlayerStats, match_format: MatchFormat):
        ...
        self.odds_predictor = FastOddsPredictor()
        self.current_odds = {
    ...
    def update_odds(self):
        features = self.odds_predictor.prepare_features(self.state, self.players)
        win_probability = self.odds_predictor.predict_win_probability(features)

        self.current_odds['match_winner'] = OddsCalculator.adjust_odds(
            self.current_odds['match_winner'], win_probability
        )
        # You can add similar adjustments for set_winner and game_winner if needed
